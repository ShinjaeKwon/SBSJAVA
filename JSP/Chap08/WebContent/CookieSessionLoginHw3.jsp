...
<body>
	...
	<form action="CookieSessionSetHw3.jsp" method="post">
		...
		<table border="1">
			...
			<tr>
				<td colspan="2" align="right">
					<input type="checkbox" name="rememberId" value="keep" <%=(rememberId== null) ? "" : "checked=\"checked\""%> />자동로그인
			<tr>
	...
	<% 
		}else if(rememberId.equals("keep")){ %>
			<form action="CookieSessionSetHw3.jsp" method="post">
				<input type="hidden" name="id" value="<%=(id == null ? "" : id ) %>"></td>
				<input type="hidden" name="pw" value="<%=(pw == null ? "" : pw) %>"></td>
				<input type="hidden" name="rememberId" value="<%=(rememberId.equals("keep")? "keep" : "temp")%>" />
			</form>
	<% 
			response.sendRedirect("http://localhost:8080/Chap08/CookieSessionSetHw3.jsp");
		}
	%>
</body>
</html>