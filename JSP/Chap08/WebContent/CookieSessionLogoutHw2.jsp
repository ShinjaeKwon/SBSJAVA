...
<body>
	...
	<%
		...
		if(cookies != null && cookies.length > 0){ //쿠기값들이 존재한다면 
			for(int i=0; i<cookies.length; i++){
				...
				if(cookies[i].getName().equals("rememberId")){ //기억정보가 keepId/no/ keepBoth
	...
	<%
		//keep이면 id값 그대로 넘김
		if(rememberId != null && rememberId.equals("no")){ //기억 x
		...
		}else if(rememberId != null && rememberId.equals("keepId")){ //아이디만 기억 
			if(cookies != null && cookies.length > 0){ //쿠키값이 존재한다면 
				for(int i=0; i<cookies.length; i++){
					if(cookies[i].getName().equals("pw")){ 
						cookies[i].setMaxAge(0); //pw만 종료시킴
						response.addCookie(cookies[i]); // 갱신
					}
				}
			}
