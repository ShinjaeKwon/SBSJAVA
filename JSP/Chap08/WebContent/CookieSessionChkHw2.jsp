...
<body>
	<%
		...
		if(cookies != null && cookies.length > 0){ //받아온 쿠키들이 존재하면 
			for(int i=0; i<cookies.length; i++){
				...
				if(cookies[i].getName().equals("pw")){ //pw 이름을 가진 쿠기가 존재하면
					pw = cookies[i].getValue(); //pw 저장
				}
			}
		}
	%>
	...
	%>
		...
		로그인 아이디 기억 상태를 [
		<%
			if(rememberId.equals("no")){
				out.print("저장하지 않음");
			}else if(rememberId.equals("keepId")){
				out.print("아아디만 저장");
			}else{
				out.print("아이디/비밀번호 모두 저장");
			}
		%>
		
		]으로 설정하셨습니다.</h3>
		<form action="CookieSessionLogoutHw2.jsp" method="post">
	...
	%>
		...
		로그인하여 주시기 바랍니다.	</h3>
		<form action="CookieSessionLoginHw2.jsp" method="post">
